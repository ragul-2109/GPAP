const SuperSubscriptionsManager = (function() {
    ...
    function openAllInvoicesModal() {
        getModal('allInvoicesModal').show();
    }

    function openCustomPlanModal() {
        getModal('customPlanModal').show();
    }

    function openEditTierModal(btn) {
        const card = btn.closest('.lux-card');
        const tierName = card.querySelector('h5').innerText;
        document.getElementById('editTierTitle').innerText = `Edit ${tierName} Tier`;
        getModal('editTierModal').show();
    }

    function downloadInvoice(invoiceId) {
        showToast(`Generating PDF for ${invoiceId}...`, 'success');
        setTimeout(() => {
...
window.SuperSubscriptionsManager = SuperSubscriptionsManager;
